...
TITLES={'cache':'应用缓存','build_cache':'代码生成缓存','build_binary':'编译中间二进制',
        'temporary':'较早的临时文件/转储','logs':'较早的应用日志','test_database':'测试数据库生成文件'}
IMPACTS={'cache':'退出对应应用；资源、依赖或索引需重新加载、下载或生成',
         ...
         'test_database':'停止状态且已确认可重建的实例；执行前复核进程与静置时间'}
...
def main():
    ...
    assert plan['approved'] is False and plan['execution_implemented'] is False
    ...
    lines=['# C、D 盘完整扫描与待选清理清单','',
           ...
           f"合计候选约 **{human(sum(reclaim.values()))}**，按应用和用途拆成 {len(plan['groups'])} 组。候选只涵盖逐文件清单，不能整删上级目录。","",
           ...
           f'初筛候选中 {ignored_sources} 个 Git 已跟踪文件被保留；仓库内只有“未跟踪且被忽略”的文件才可能通过。配置、源码、凭据、聊天存储、安装环境、多个硬链接、读取失败或扫描后变化的文件均不进入对应的释放估算。','',
           '## 较大的待选项','',
           ...
           f"全部编号、精确目录、Git 依据、类型与影响见 {local_link(run,'全部清理选项.md')}。可以按编号选择，也可以指定应用名；当前所有选项均未批准。",'',
           '## 过时、旧版本和临时文件','',
           ...
           '扫描与分析命令不包含删除执行功能。复用方式见仓库 [README.md](../../README.md)。','']
