# PC_Clear：Windows 磁盘分析与清理

扫描 C、D 盘所有可访问的普通文件，按应用、目录布局、Git 仓库、文件类型、修改年龄与运行状态分析占用。**默认只分析；所有可清理项默认未勾选。** 在界面中勾选、预览并明确确认后，才逐文件永久清理。不自动关闭用户应用、不修改系统权限。
...
1. **全部占用**：顶部先读取 Windows 的**物理硬盘 → 分区**关系，实时显示硬盘、C/D 及系统/保留/恢复分区的容量、已用和可用空间。C、D 是同一物理硬盘上的分区时不会再被合计为两块硬盘；系统、保留和恢复分区仅展示容量，不扫描或清理。实时容量和报告中的扫描逻辑文件分开说明，避免把快照当成当前可用空间。可点击“刷新实时容量”；清理结束后也会自动刷新。若 Windows 查询失败，界面会明确切换为“扫描时容量”，不会将快照标成当前容量。
2. **应用智能分析 / 文件夹分析**：两个页签独立展示，均以“物理硬盘 → 已扫描分区”为第一层。应用树继续展开到应用、用途和分析路径；文件夹树按真实目录逐层展开，双击文件夹进入完整目录浏览。保护内容仍在统计中。
3. **目录与文件**：按盘和路径逐层浏览，也可全盘搜索文件名。单页按大小显示前 1,000 条，完整文件记录均在 SQLite 中。
4. **可清理项**：按操作方式分为“可清理内容”“需要应用内处理”和“聊天媒体需确认”。只有可清理内容和主动显示的聊天媒体可以勾选；测试数据库列为应用内整体处理，避免逐文件破坏数据。双击具体项目可查看精确范围；显示不代表已经勾选。
5. 点击 **预览已选文件**。程序复核 Git、当前规则、文件身份和运行进程，列出可执行文件、跳过原因、估算空间及完整逐文件预览文件。预览不会删除文件。
6. 阅读预览、退出相关应用，在 **执行预览** 页勾选已退出应用，并输入本次预览的确认词，再点击 **永久删除已预览文件**。确认对话框默认选择“否”。

清理是永久删除，不经过回收站。聊天媒体可能失去唯一的原图、视频或语音；聊天数据库、文档附件、配置与会话仍保留。数据库实例使用应用/数据库管理方式整体处理，此入口不部分删除数据库数据文件。

每次预览有效 15 分钟且只能执行一次。改变勾选或保护规则、重新分析都会使预览失效。未勾选、未预览、未正确确认时均无法执行。可停止扫描或清理；停止清理不会撤销已经删除的文件。执行后查看 cleanup_*.json / *.jsonl，重新扫描更新占用。
...
inventory.sqlite 包含 files、directories、gaps、metadata 表。candidate_files.jsonl 是最终逐文件可清理清单；cleanup_plan.json 的编号通过 group_key 对应清单。计划不是执行脚本，不允许按目录列表直接整删。
...
- temporary_min_days / log_min_days / aged_review_days：临时文件、日志文件进入可清理清单的静置阈值，以及旧文件复核阈值。
...
Git 查询仅为单次命令指定 safe.directory，不修改全局 Git 配置。仓库内文件必须同时未跟踪、被忽略，才可能进入可清理清单；查询失败则保留。Git 忽略规则不能证明文件无用，也不能覆盖保护规则。
...
- 可清理文件会重新获取文件信息，排除多硬链接、重解析点、不可读或扫描后变化的文件；压缩/稀疏文件修正大小。估算不等于实际簇释放量。
